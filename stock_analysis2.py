...
def main(ticker):
    ...
    if stock_data is None:
        return json.dumps({"error": "Failed to fetch stock data. Please check the ticker symbol and try again."})
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(json.dumps({"error": "Usage: python stock_analysis.py <ticker>"}))
        sys.exit(1)
